...
/*	= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
	...
	Page Name						Purpose :
	Vehicle Operation Browse.php			The purpose of this page is to browse Vehicle Operations
...
*/	
	// Load Required Include Files
		...
		//include("includes/NavFunctions.php");																		// In Header.php
		//include("includes/UserFunctions.php");																		// In Header.php
	...
	// this information is needed to program the datafields and sql statements on the fly without any user interuption
		...
		$tblkeyfield			= "maintenance_sub_vo_id";										// What is the Auto Increment Field for this table ?
		$tbldatesortfield		= "maintenance_sub_vo_date";									// What is the name of field use in date sorting ?
		$tbldatesorttable		= "tbl_maintenance_sub_v_o";									// What table  is that field part of ?
		$tbltextsortfield		= "maintenance_sub_vo_work";									// What is the name of the field used in text sorting ?
		$tbltextsorttable		= "tbl_maintenance_sub_v_o";									// What is the name of the table used for text sorting ?
		$tblarchivedfield		= "maintenance_sub_ve_archived_yn";								// What is the name of the field used to mark the record archived ?
		$tblname				= "Vehicle Events Summary Report";								// What is the name of the table ? (used on edit/summary/printer report pages)
	...
	// what columns should the datagrid display?
		...
		$adatafield[0]			= "maintenance_sub_vo_date";
		$adatafield[1]			= "maintenance_sub_vo_time";
		$adatafield[2]			= "maintenance_sub_vo_by_cb_int";
		$adatafield[3]			= "maintenance_sub_voe_cb_int";
		$adatafield[4]			= "maintenance_sub_vov_cb_int";
		$adatafield[5]			= "maintenance_sub_vo_miles";
		$adatafield[6]			= "maintenance_sub_vo_hours";
		$adatafield[7]			= "maintenance_sub_vo_work";
		$adatafield[8]			= "maintenance_sub_vo_problems";
		$adatafield[9]			= "maintenance_sub_vo_archived_yn";
	// in each array specified above, what table does that field come from?
		$adatafieldtable[0]		= "tbl_maintenance_sub_v_o";
		$adatafieldtable[1]		= "tbl_maintenance_sub_v_o";
		$adatafieldtable[2]		= "tbl_maintenance_sub_v_o";
		$adatafieldtable[3]		= "tbl_maintenance_sub_v_o";	
		$adatafieldtable[4]		= "tbl_maintenance_sub_v_o";
		$adatafieldtable[5]		= "tbl_maintenance_sub_v_o";
		$adatafieldtable[6]		= "tbl_maintenance_sub_v_o";
		$adatafieldtable[7]		= "tbl_maintenance_sub_v_o";
		$adatafieldtable[8]		= "tbl_maintenance_sub_v_o";
		$adatafieldtable[9]		= "tbl_maintenance_sub_v_o";		
	// do you want the user ot be able to click on the information and have something happen?
		...
		$adatafieldid[2]		= "maintenance_sub_vo_by_cb_int";
		$adatafieldid[3]		= "maintenance_sub_voe_cb_int";
		$adatafieldid[4]		= "maintenance_sub_vov_cb_int";
	...
	// what do you want to name the columns
		...
		$aheadername[3]			= "Event";
		$aheadername[4]			= "Vehicle";
		$aheadername[5]			= "Miles";
		$aheadername[6]			= "Hours";
		$aheadername[7]			= "Work";
		$aheadername[8]			= "Problems";
		$aheadername[9]			= "Archived";
	// Any special comments to make after the input box?
		$ainputcomment[0]		= "( mm/dd/yyyy )";
		$ainputcomment[1]		= "( 24 hour time )";
		$ainputcomment[2]		= "( Select from list )";
		$ainputcomment[3]		= "( Select from list )";
		$ainputcomment[4]		= "( Select from list )";
	...
	// if the input type is a SELECT type you should define the name of the function you want to call to load into that select statement
		...
		$adataselect[3]			= "maintenanceeventscombobox";
		$adataselect[4]			= "inventoryvehiclescombobox";
		$adataselect[5]			= "";
		$adataselect[6]			= "";
		$adataselect[7]			= "";
		$adataselect[8]			= "";
...
<table border="0" width="100%" id="tblbrowseformtable" cellspacing="0" cellpadding="0">
	<tr>
		<td width="10" style="border-left-width: 1px; border-right-width: 1px; border-top-width: 1px; border-bottom: 1px solid #3b5998">&nbsp;</td>
		<td align="left" valign="bottom" class="formheader">
			<?
			getnameofmenuitemid($strmenuitemid, "long", 4, "#ffffff");
			?>
			</td>
		<td align="right" valign="bottom" class="formpurpose">
			(
			<?
			getpurposeofmenuitemid($strmenuitemid, 1, "#6d84b4");
			?>
			)
			</td>
		</tr>
	<tr>
		<td colspan="3" bgcolor="#d8dfea" style="border-left: 1px solid #6d84b4; border-right: 1px solid #6d84b4; border-top-width: 1px; border-bottom: 1px solid #6d84b4; padding: 0" align="center" valign="top">
			<table border="0" width="100%" cellspacing="3" cellpadding="5" id="table2" height="10">
				...
				<tr>
					<td style="font-family: arial narrow; font-size: 10pt; color: #3b5998; border: 1px solid #3b5998" align="center">
						<?
				...
				<tr>
					<td style="font-family: arial narrow; font-size: 10pt; color: #3b5998">
						...
						if (mysqli_connect_errno()) {
								...
								//where tbl_duty_log.duty_log_date >= '".$tmpfrmstartdate."' and tbl_duty_log.duty_log_date <= '".$tmpfrmenddate."'";
								
								//if ($frmtextlike == "") {
										//do not add text where to sql statement
								//	}
									//else {
										//$nsql = " and tbl_duty_log.duty_log_comments like '".$frmtextlike."%'";
										//$sql = $sql.$nsql;
										//echo $sql;
									//}
				...
				<tr>
					<td>
						<table cellspacing="0" width="100%">
							<tr>
								<td style="border-left:1px solid #3b5998; border-right:1px solid #3b5998; border-top:1px solid #3b5998; border-bottom:1px dotted #3b5998; font-family: arial narrow; font-size: 10pt; color: #3b5998; background: #ffffff">
									The following options are avilable to you
									</td>
								</tr>
							<tr>
								<td align="right" style="border-left:1px solid #3b5998; border-right:1px solid #3b5998; border-bottom:1px solid #3b5998; padding:3px; font-family: arial narrow; font-size: 10pt; color: #3b5998; background: #ffffff">
									<table>
										<tr>
											<?
											$encoded = urlencode($sql);
											?>
											<td style="font-family: arial narrow; font-size: 10pt; color: #ffffff; border: 1px solid #d8dfea; cursor: hand;" bgcolor="#3b5998" align="center" onclick="window.open('/browse_report_general.php?frmurl=<?=$encoded;?>&menuitemid=<?=$strmenuitemid?>&aheadername=<?=$straheadername;?>&adatafield=<?=$stradatafield;?>&tblkeyfield=<?=$tblkeyfield;?>&tbldatesortfield=<?=$tbldatesortfield;?>&tbldatesorttable=<?=$tbldatesorttable;?>&tbltextsortfield=<?=$tbltextsortfield;?>&tbltextsorttable=<?=$tbltextsorttable;?>&adatafieldtable=<?=$stradatafieldtable;?>&adatafieldid=<?=$stradatafieldid;?>&adataspecial=<?=$stradataspecial;?>&adataselect=<?=$stradataselect?>&tblarchivedfield=<?=$tblarchivedfield?>','printerfriendlyreport','width=750,height=550');">
						...
						<table border="0" width="100%" id="table1" cellpadding="0" cellspacing="1" bgcolor="#3b5998" style="border-collapse: collapse">
							...
							<tr>
								...
								<td align="center" class="formresults">
									<table border="1" width="50" cellspacing="0" id="table1" style="border: 1px solid #3b5998" cellpadding="0">
										<tr>
