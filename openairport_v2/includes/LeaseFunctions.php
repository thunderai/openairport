...
function leasetypescombobox($suppliedid, $archived, $nameofinput, $showcombobox, $default, $nameoffieldtochange) {
	...
	$sql = "SELECT * FROM tbl_general_tblrlshp  ";											// start the SQL Statement with the common syntax
	...
	if ($suppliedid=="all") {																		// if supplied 'all' for the menu_id so the following
			...
			$nsql = "WHERE `tbl_gtr_t_id` = ".$suppliedid." ";										// if supplied a menu_id, then add it to the SQL Statement
	...
	if ($archived == "all") {																	// if supplied 'all' for the archived variable do the following
			...
			if ($archived=="yes") {																// If archived is 'yes' then
					if ($tmp_flagger==0) {
							$nsql = "WHERE tbl_gtr_t_archived_yn = 1 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND tbl_gtr_t_archived_yn = 1 ";
							$sql = $sql.$nsql;
						}
				}
				else {
					if ($tmp_flagger==0) {
							$nsql = "WHERE tbl_gtr_t_archived_yn = 0 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND tbl_gtr_t_archived_yn = 0 ";
							$sql = $sql.$nsql;
						}
				}
		}
...
function leasetypescomboboxnoajax($suppliedid, $archived, $nameofinput, $showcombobox, $default) {
	// $suppliedid		, is the number of the group to do the search for ;
	// $archived		, do you want to list all menu items, or just the archived ones;
	// $nameofinout		, what is the name of the select box that 'could' be ceated by this function;
	// $showcombobox	, Do you want to show the combo box select input style or just the text without the input box;
...
function leasetermscombobox($suppliedid, $archived, $nameofinput, $showcombobox, $default) {
	// $suppliedid		, is the number of the group to do the search for ;
	// $archived		, do you want to list all menu items, or just the archived ones;
	// $nameofinout		, what is the name of the select box that 'could' be ceated by this function;
	// $showcombobox	, Do you want to show the combo box select input style or just the text without the input box;
	...
	$sql = "SELECT * FROM tbl_leases_terms  ";											// start the SQL Statement with the common syntax
	...
	if ($suppliedid=="all") {																		// if supplied 'all' for the menu_id so the following
			...
			$nsql = "WHERE `leases_terms_id` = ".$suppliedid." ";										// if supplied a menu_id, then add it to the SQL Statement
			$sql = $sql.$nsql;																	// combine the nsql and sql strings
			$tmp_flagger = 1;																	// important to tell the procedures below this happened
		}
	...
	if ($archived == "all") {																	// if supplied 'all' for the archived variable do the following
			...
			if ($archived=="yes") {																// If archived is 'yes' then
					if ($tmp_flagger==0) {
							$nsql = "WHERE leases_terms_archived_yn = 1 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND leases_terms_archived_yn = 1 ";
							$sql = $sql.$nsql;
						}
				}
				else {
					if ($tmp_flagger==0) {
							$nsql = "WHERE leases_terms_archived_yn = 0 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND leases_terms_archived_yn = 0 ";
							$sql = $sql.$nsql;
						}
				}
		}
	...
	<SELECT class="Commonfieldbox" name="<?=$nameofinput?>">
		<?
					...
					while ($objfields = mysqli_fetch_array($objrs_support, MYSQLI_ASSOC)) {
							$tmpsuppliedid 		= $objfields['leases_terms_id'];
							$tmpsuppliedname 	= $objfields['leases_terms_name'];
							$tmpsuppliedarch		= $objfields['leases_terms_archived_yn'];
...
function leaseitemtypecombobox($suppliedid, $archived, $nameofinput, $showcombobox, $default,$tabletypeid) {

	//echo "Supplied ID|".$suppliedid."| was";
	// $suppliedid		, is the number of the group to do the search for ;
	// $archived		, do you want to list all menu items, or just the archived ones;
	// $nameofinout		, what is the name of the select box that 'could' be ceated by this function;
	// $showcombobox	, Do you want to show the combo box select input style or just the text without the input box;
	...
	$sql = "SELECT * FROM tbl_general_tblrlshp WHERE tbl_gtr_t_id = '".$tabletypeid."' ";
	$objconn_support = mysqli_connect("localhost", "webuser", "limitaces", "openairport");
	if (mysqli_connect_errno()) {
			...
			if ($objrs_support) {
					$number_of_rows = mysqli_num_rows($objrs_support);
					while ($objfields = mysqli_fetch_array($objrs_support, MYSQLI_ASSOC)) {
							$tmptablename	= $objfields['tbl_gtr_t_tablename'];
							//echo "<br> Get information to list objects from table ".$tmptablename."<br>";
						}
						mysqli_free_result($objrs_support);
						mysqli_close($objconn_support);
				}
		}
		$tmpitemname	=	$tmptablename($suppliedid, "all", "leases_type_id", $showcombobox, $default);
	}
?>
