<?php
function part139303_c_conditioncomboboxwall($suppliedid, $archived, $nameofinput, $showcombobox, $default) {
	...
	$sql = "SELECT * FROM tbl_139_303_c_sub_c ";													// start the SQL Statement with the common syntax
	...
	if ($archived == "all") {																	// if supplied 'all' for the archived variable do the following
			...
			if ($archived=="yes") {																// If archived is 'yes' then
					if ($tmp_flagger==0) {
							$nsql = "WHERE tbl_139_303_c_sub_c.condition_archived_yn = 1 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND tbl_139_303_c_sub_c.condition_archived_yn = 1 ";
							$sql = $sql.$nsql;
						}
				}
				else {
					if ($tmp_flagger==0) {
							$nsql = "WHERE tbl_139_303_c_sub_c.condition_archived_yn = 0 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND tbl_139_303_c_sub_c.condition_archived_yn = 0 ";
							$sql = $sql.$nsql;
						}
				}
		}
	...
	<SELECT class="Commonfieldbox" NAME="<?php echo $nameofinput?>" ID="<?php echo $nameofinput?>">
		...
		<option 
				...
				SELECTED
								...
								if ($showcombobox=="show") {
										// Limit the supplied name to the first x chars.
										$tmp_suppliedname_ax 	= str_split($tmpsuppliedname,50);
										$tmp_suppliedname_x		= $tmp_suppliedname_ax[0]."...";
										?>
				value="<?php echo $tmpsuppliedid;?>"><?php echo $tmp_suppliedname_x;?></option>
										<?php 
									}
									else {
										?>
