...
/*	= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
	...
	Page Name						Purpose :
	Equipment Functions.php				The purpose of this page is to load Functions used in the Equipment Module of the system
	...
	= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
*/
function equipment_combobox($suppliedid, $archived, $nameofinput, $showcombobox, $default) {
	...
	$sql = "SELECT * FROM tbl_inventory_sub_e ";										// start the SQL Statement with the common syntax
	...
	if ($suppliedid=="all") {																	// if supplied 'all' for the menu_id so the following
			...
			$nsql = "WHERE `equipment_id` = ".$suppliedid." ";								// if supplied a menu_id, then add it to the SQL Statement
	...
	if ($archived == "all") {																	// if supplied 'all' for the archived variable do the following
			...
			if ($archived=="yes") {																// If archived is 'yes' then
					if ($tmp_flagger==0) {
							$nsql = "WHERE equipment_archived_yn = -1 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND equipment_archived_yn = -1 ";
							$sql = $sql.$nsql;
						}
				}
				else {
					if ($tmp_flagger==0) {
							$nsql = "WHERE equipment_archived_yn = 0 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND equipment_archived_yn = 0 ";
							$sql = $sql.$nsql;
						}
				}
		}
	...
	<SELECT class="Commonfieldbox" name="<?=$nameofinput?>">
					...
					while ($objfields = mysqli_fetch_array($objrs_support, MYSQLI_ASSOC)) {
							$tmpsuppliedid 		= $objfields['equipment_id'];
							$tmpsuppliedname 	= $objfields['equipment_name'];
							$tmpsuppliedarch		= $objfields['equipment_archived_yn'];
...
function equipment_by_types_combobox($suppliedid, $archived, $nameofinput, $showcombobox, $default) {
	// $suppliedid		, is the number of the group to do the search for ;
	// $archived		, do you want to list all menu items, or just the archived ones;
	// $nameofinout		, what is the name of the select box that 'could' be ceated by this function;
	// $showcombobox	, Do you want to show the combo box select input style or just the text without the input box;
	...
	$sql = "SELECT * FROM tbl_inventory_sub_e INNER JOIN tbl_inventory_sub_e_sub_t ON equipment_type_cb_int = equipment_sub_type_id ";										// start the SQL Statement with the common syntax
	...
	if ($suppliedid=="all") {																	// if supplied 'all' for the menu_id so the following
			...
			$nsql = "WHERE `equipment_sub_type_id` = ".$suppliedid." ";								// if supplied a menu_id, then add it to the SQL Statement
			$sql = $sql.$nsql;																	// combine the nsql and sql strings
			$tmp_flagger = 1;																	// important to tell the procedures below this happened
		}
...
function equipmenttypescombobox($suppliedid, $archived, $nameofinput, $showcombobox, $default) {
	// $suppliedid		, is the number of the group to do the search for ;
	// $archived		, do you want to list all menu items, or just the archived ones;
	// $nameofinout		, what is the name of the select box that 'could' be ceated by this function;
	// $showcombobox	, Do you want to show the combo box select input style or just the text without the input box;
	...
	$sql = "SELECT * FROM tbl_inventory_sub_e_sub_t  ";											// start the SQL Statement with the common syntax
	...
	if ($archived == "all") {																	// if supplied 'all' for the archived variable do the following
			...
			if ($archived=="yes") {																// If archived is 'yes' then
					if ($tmp_flagger==0) {
							$nsql = "WHERE equipment_sub_type_archived_yn = 1 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND equipment_sub_type_archived_yn = 1 ";
							$sql = $sql.$nsql;
						}
				}
				else {
					if ($tmp_flagger==0) {
							$nsql = "WHERE equipment_sub_type_archived_yn = 0 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND equipment_sub_type_archived_yn = 0 ";
							$sql = $sql.$nsql;
						}
				}
		}
	...
	<SELECT class="Commonfieldbox" name="<?=$nameofinput?>">
					...
					while ($objfields = mysqli_fetch_array($objrs_support, MYSQLI_ASSOC)) {
							$tmpsuppliedid 		= $objfields['equipment_sub_type_id'];
							$tmpsuppliedname 	= $objfields['equipment_sub_type_name'];
							$tmpsuppliedarch	= $objfields['equipment_sub_type_archived_yn'];
...
function equipment_with_eventscomboboxs($suppliedid, $archived, $nameofinput_manufacturer, $nameofinput_model, $showcombobox, $default) {
	// $suppliedid		, is the number of the group to do the search for ;
	// $archived		, do you want to list all menu items, or just the archived ones;
	// $nameofinout		, what is the name of the select box that 'could' be ceated by this function;
	// $showcombobox	, Do you want to show the combo box select input style or just the text without the input box;
	...
	$sql = "SELECT * FROM `tbl_inventory_sub_e` 
			INNER JOIN tbl_maintenance_sub_v_e 	ON equipment_id = maintenance_sub_vev_cb_int 
			INNER JOIN tbl_organization_main		ON equipment_manufac_cb_int = Organizations_id 
			INNER JOIN tbl_maintenance_sub_e		ON mxevent_id = maintenance_sub_vee_cb_int 
			ORDER BY org_name, equipment_modelyear, equipment_name, mxevent_name ";
			...
			$objconn_support = mysqli_connect("localhost", "webuser", "limitaces", "openairport");
				...
				if (mysqli_connect_errno()) {
						...
						if ($objrs_support) {
								...
								//printf("result set has %d rows. \n", $number_of_rows);
								?>			
	<select class="Commonfieldbox" NAME="<?=$nameofinput_manufacturer;?>" SIZE=10" ONCHANGE="manuselected(this);" >
								<?
								...
								while ($objfields = mysqli_fetch_array($objrs_support, MYSQLI_ASSOC)) {
										...
										if ($sLastManufacturer <> $objfields['equipment_id'] ) {
												// if so, add an entry to the first listbox
												$sLastManufacturer 	= $objfields['equipment_id'];
												$sMan 			= $objfields['org_name'];
												$sName 			= $objfields['equipment_name'];
												$sModel 			= $objfields['equipment_modelnumber'];
												$sYear 			= $objfields['equipment_modelyear'];
		...
		<OPTION VALUE="<?=$sLastManufacturer;?>"><?=$sMan;?>&nbsp;<?=$sName;?>&nbsp;<?=$sModel;?>&nbsp;(<?=$sYear;?>)</OPTION>
												...
												$sJavaScript = $sJavaScript."} \n if (elem.options[elem.selectedIndex].value==".$objfields['equipment_id']."){ \n";
    												}
