...
/*	= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
	...
	Page Name						Purpose :
	Building Functions.php				The purpose of this page is to load Functions used in theBuilding Module of the system
...
function Buildingtypescombobox($suppliedid, $archived, $nameofinput, $showcombobox, $default) {
	...
	$sql = "SELECT * FROM tbl_inventory_sub_b_sub_t  ";											// start the SQL Statement with the common syntax
	...
	if ($suppliedid=="all") {																		// if supplied 'all' for the menu_id so the following
			...
			$nsql = "WHERE `buildings_sub_type_id` = ".$suppliedid." ";										// if supplied a menu_id, then add it to the SQL Statement
	...
	if ($archived == "all") {																	// if supplied 'all' for the archived variable do the following
			...
			if ($archived=="yes") {																// If archived is 'yes' then
					if ($tmp_flagger==0) {
							$nsql = "WHERE buildings_sub_type_archived_yn = 1 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND buildings_sub_type_archived_yn = 1 ";
							$sql = $sql.$nsql;
						}
				}
				else {
					if ($tmp_flagger==0) {
							$nsql = "WHERE buildings_sub_type_archived_yn = 0 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND buildings_sub_type_archived_yn = 0 ";
							$sql = $sql.$nsql;
						}
				}
		}
	...
	<SELECT class="Commonfieldbox" name="<?=$nameofinput?>">
					...
					while ($objfields = mysqli_fetch_array($objrs_support, MYSQLI_ASSOC)) {
							$tmpsuppliedid 		= $objfields['buildings_sub_type_id'];
							$tmpsuppliedname 	= $objfields['buildings_sub_type_name'];
							$tmpsuppliedarch	= $objfields['buildings_sub_type_archived_yn'];
...
function building_with_eventscomboboxs($suppliedid, $archived, $nameofinput_manufacturer, $nameofinput_model, $showcombobox, $default) {
	// $suppliedid		, is the number of the group to do the search for ;
	// $archived		, do you want to list all menu items, or just the archived ones;
	// $nameofinout		, what is the name of the select box that 'could' be ceated by this function;
	// $showcombobox	, Do you want to show the combo box select input style or just the text without the input box;
	...
	$sql = "SELECT * FROM `tbl_inventory_sub_b` 
			INNER JOIN tbl_maintenance_sub_b_e 	ON buildings_id = maintenance_sub_vev_cb_int 
			INNER JOIN tbl_organization_main		ON buildings_manufac_cb_int = Organizations_id 
			INNER JOIN tbl_maintenance_sub_e		ON mxevent_id = maintenance_sub_vee_cb_int 
			ORDER BY org_name, buildings_modelyear, buildings_name, mxevent_name ";
	...
	<select class="Commonfieldbox" NAME="<?=$nameofinput_manufacturer;?>" SIZE="10" STYLE="width: 380px" ONCHANGE="manuselected(this);" >
								...
								while ($objfields = mysqli_fetch_array($objrs_support, MYSQLI_ASSOC)) {
										...
										if ($sLastManufacturer <> $objfields['buildings_id'] ) {
												// if so, add an entry to the first listbox
												$sLastManufacturer 	= $objfields['buildings_id'];
												$sMan 			= $objfields['org_name'];
												$sName 			= $objfields['buildings_name'];
												$sModel 			= $objfields['buildings_modelnumber'];
												$sYear 			= $objfields['buildings_modelyear'];
		...
		<OPTION VALUE="<?=$sLastManufacturer;?>"><?=$sMan;?>&nbsp;<?=$sName;?>&nbsp;<?=$sModel;?>&nbsp;(<?=$sYear;?>)</OPTION>
												...
												$sJavaScript = $sJavaScript."} \n if (elem.options[elem.selectedIndex].value==".$objfields['buildings_id']."){ \n";
    												}
