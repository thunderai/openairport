...
*/	
	...
	// this information is needed to program the datafields and sql statements on the fly without any user interuption
		...
		$tblkeyfield			= "timesheet_id";												// What is the Auto Increment Field for this table ?
		$tbldatesortfield		= "";															// What is the name of field use in date sorting ?
		$tbldatesorttable		= "tbl_timesheets_main";											// What table  is that field part of ?
		$tbltextsortfield		= "";												// What is the name of the field used in text sorting ?
		$tbltextsorttable		= "tbl_timesheets_main";											// What is the name of the table used for text sorting ?
		$tblarchivedfield		= "timesheet_archived_yn";										// What is the name of the field used to mark the record archived ?
		$tblname				= "TimeSheet Summary Report";									// What is the name of the table ? (used on edit/summary/printer report pages)
	...
	// what columns should the datagrid display?
		...
		$adatafield[0]			= "timesheet_systemuser_id_cb_int";
		$adatafield[1]			= "timesheet_month_cb_int";
	// in each array specified above, what table does that field come from?
		$adatafieldtable[0]		= "tbl_timesheets_main";
		$adatafieldtable[1]		= "tbl_timesheets_main";
	// do you want the user ot be able to click on the information and have something happen?
		...
		$adatafieldid[0]		= "timesheet_systemuser_id_cb_int";
		$adatafieldid[1]		= "timesheet_month_cb_int";
	...
	// Any special comments to make after the input box?
		$ainputcomment[0]		= "The author of this timesheet is automatically set to the default value";
		$ainputcomment[1]		= "Please select from the list to the right, the month this time period begin in";
	...
	// if the input type is a SELECT type you should define the name of the function you want to call to load into that select statement
		...
		$adataselect[1]			= "timesheetmonthscombobox_limit";
...
if (!isset($_POST["formsubmit"])) {
	...
	$sadataselect			= str_replace("\"","|",$sadataselect);									// Dont Touch
		...
		// $dutylogevent		= "New TimeSheet: ".$lastid." Was Created";
		
		// autodutylogentry($tmpsqldate,$tmpsqltime,$tmpsqlauthor,$dutylogevent);
		?>

			<form name="redirect" action="timesheet_main_build.php" method="POST">
				&nbsp;<input class="combobox" type="hidden" size="1" name="redirect2">
												<input type="hidden" name="parentid" 			value="<?=$lastid;?>">
												<input type="hidden" name="entryby" 			value="<?=$_POST[$adatafield[0]];?>">
												<input type="hidden" name="monthid" 			value="<?=$_POST[$adatafield[1]];?>">
												<input type="hidden" name="editpage" 			value="<?=$functioneditpage;?>">
												<input type="hidden" name="summarypage" 		value="<?=$functionsummarypage;?>">
												<input type="hidden" name="printerpage" 		value="<?=$functionprinterpage;?>">
