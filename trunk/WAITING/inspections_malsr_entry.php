...
*/	
	...
	// this information is needed to program the datafields and sql statements on the fly without any user interuption
		...
		$tblkeyfield			= "malsr_id";												// What is the Auto Increment Field for this table ?
		$tbldatesortfield		= "malsr_date";															// What is the name of field use in date sorting ?
		$tbldatesorttable		= "tbl_inspections_malsr_main";											// What table  is that field part of ?
		$tbltextsortfield		= "malsr_remarks";												// What is the name of the field used in text sorting ?
		$tbltextsorttable		= "tbl_inspections_malsr_main";											// What is the name of the table used for text sorting ?
		$tblarchivedfield		= "malsr_archived_yn";										// What is the name of the field used to mark the record archived ?
		$tblname				= "M.A.L.S.R. Summary Report";									// What is the name of the table ? (used on edit/summary/printer report pages)
	...
	// what columns should the datagrid display?
		...
		$adatafield[0]			= "malsr_date";
		$adatafield[1]			= "malsr_time";
		$adatafield[2]			= "malsr_inspected_by_cb_int";
		$adatafield[3]			= "malsr_obstrution_yn";
		$adatafield[4]			= "malsr_obstrution_txt";
		$adatafield[5]			= "malsr_visibility_yn";
		$adatafield[6]			= "malsr_visibility_txt";
		$adatafield[7]			= "malsr_remarks";
	// in each array specified above, what table does that field come from?
		...
		$adatafieldtable[4]		= "tbl_aircraft_main";
		$adatafieldtable[5]		= "tbl_aircraft_main";
		$adatafieldtable[6]		= "tbl_aircraft_main";
		$adatafieldtable[7]		= "tbl_aircraft_main";	
	// do you want the user ot be able to click on the information and have something happen?
		...
		$adatafieldid[5]		= "aircraft_type_cb_int";
	...
	// what do you want to name the columns
		...
		$aheadername[3]			= "Obstruction Problems";
		$aheadername[4]			= "Obstruction Comments";
		$aheadername[5]			= "Any Lighting Problems";
		$aheadername[6]			= "Lighting Comments";
		$aheadername[7]			= "General Remarks";
	// Any special comments to make after the input box?
		$ainputcomment[0]		= "Please enter the date of the inspection<br><br><br>Date should be in mm/dd/YYYY format";
		$ainputcomment[1]		= "Please enter the time of the inspection<br><br><br>Time should be in HH:MM:SS format";
		$ainputcomment[2]		= "Please select the person to conducted the inspection";
		$ainputcomment[3]		= "If there where problems found during the inspection, check this box";
		$ainputcomment[4]		= "Please enter any comments about this item here";
		$ainputcomment[5]		= "If there where problems found during the inspection, check this box";
		$ainputcomment[6]		= "Please enter any comments about this item here";
		$ainputcomment[7]		= "( checked = archived )";
...
if (!isset($_POST["formsubmit"])) {
	...
	$sadataselect			= str_replace("\"","|",$sadataselect);									// Dont Touch
		...
		$dutylogevent	= "New MALSR Inspection ID: ".$lastid." Was Created";
		
		autodutylogentry($tmpsqldate,$tmpsqltime,$tmpsqlauthor,$dutylogevent);
		
		if (!isset($_POST[$adatafield[3]])) {
				// There is no value in the Visual Obstructions, so dont send email
			}
			else {
				$sendemail = 1;
			}
		if (!isset($_POST[$adatafield[5]])) {
				// There is no value in the Visual Obstructions, so dont send email
			}
			else {
				$sendemail = 1;
		...
		if ($sendemail==1) {
				// Do email procedures
				$subject	= "MALSR Inspection (Problems Found)";
				$emailbody = "A MALSR Inspection in Watertown South Dakota was conducted on ".$_POST[$adatafield[0]]." at ".$_POST[$adatafield[1]]." \n \n 
				
				Visial Obstructions : ".$_POST[$adatafield[3]]." ".$_POST[$adatafield[4]]." \n 
				Lighting Problems : ".$_POST[$adatafield[5]]." " .$_POST[$adatafield[6]]." \n \n
				
				General Remarks : ".$_POST[$adatafield[7]]." ";
				

				sendreportbyemail("william.a.campbell@faa.gov","New MALSR Inspection Conducted",$emailbody);
			}
		?>
