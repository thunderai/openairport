...
function part139339typescomboboxwall($suppliedid, $archived, $nameofinput, $showcombobox, $default) {
	...
	if ($archived == "all") {																	// if supplied 'all' for the archived variable do the following
			...
			if ($archived=="yes") {																// If archived is 'yes' then
					if ($tmp_flagger==0) {
							...
							$nsql = "AND tbl_139_339_sub_t.inspection_type_archived_yn = -1 ";
							$sql = $sql.$nsql;
						}
				}
				else {
					if ($tmp_flagger==0) {
							$nsql = "WHERE tbl_139_339_sub_t.inspection_type_archived_yn = 0 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND tbl_139_339_sub_t.inspection_type_archived_yn = 0 ";
							$sql = $sql.$nsql;
						}
				}
		}
	...
	<SELECT class="Commonfieldbox" name="<?php echo $nameofinput?>" ID="<?php echo $nameofinput?>" >
		<option value="all">All FiCON Types</option>
					<?php 
						}
