...
*/	
	...
	// this information is needed to program the datafields and sql statements on the fly without any user interuption
		...
		$tblkeyfield			= "fuelflow_id";												// What is the Auto Increment Field for this table ?
		$tbldatesortfield		= "fuelflow_date";												// What is the name of field use in date sorting ?
		$tbldatesorttable		= "tbl_fuelflowoperations_main";								// What table  is that field part of ?
		$tbltextsortfield		= "";															// What is the name of the field used in text sorting ?
		$tbltextsorttable		= "tbl_fuelflowoperations_main";								// What is the name of the table used for text sorting ?
		$tblarchivedfield		= "fuelflow_archived_yn";										// What is the name of the field used to mark the record archived ?
		$tblname				= "Fuel Flow Operations Summary Report";						// What is the name of the table ? (used on edit/summary/printer report pages)
	...
	// what columns should the datagrid display?
		...
		$adatafield[0]			= "fuelflow_date";
		$adatafield[1]			= "fuelflow_time";
		$adatafield[2]			= "fuelflow_author_cb_by_int";
		$adatafield[3]			= "fuelflow_fbo_cb_int";
		$adatafield[4]			= "fuelflow_tank_cb_int";
		$adatafield[5]			= "fuelflow_grossgallons";
	// in each array specified above, what table does that field come from?
		$adatafieldtable[0]		= "tbl_fuelflowoperations_main";
		$adatafieldtable[1]		= "tbl_fuelflowoperations_main";
		$adatafieldtable[2]		= "tbl_fuelflowoperations_main";
		$adatafieldtable[3]		= "tbl_fuelflowoperations_main";
		$adatafieldtable[4]		= "tbl_fuelflowoperations_main";
		$adatafieldtable[5]		= "tbl_fuelflowoperations_main";			
	// do you want the user ot be able to click on the information and have something happen?
		...
		$adatafieldid[2]		= "fuelflow_author_cb_by_int";
		$adatafieldid[3]		= "fuelflow_fbo_cb_int";
		$adatafieldid[4]		= "fuelflow_tank_cb_int";
	...
	// Any special comments to make after the input box?
		...
		$ainputcomment[5]		= "( Enter the Number of gallons in or out of the tank in GROSS i.e. -45 or 45 are acceptable )";
	...
	// if the input type is a SELECT type you should define the name of the function you want to call to load into that select statement
		...
		$adataselect[3]			= "organizationcomboboxgettype";
...
if (!isset($_POST["formsubmit"])) {
	...
	$sadataselect			= str_replace("\"","|",$sadataselect);									// Dont Touch
		...
		$dutylogevent	= "New FuelFlow Operation ".$lastid." Was Created";
		
		autodutylogentry($tmpsqldate,$tmpsqltime,$tmpsqlauthor,$dutylogevent);
		
		$tankid				= $_POST[$adatafield[4]];
		$gallonsexchanged	= $_POST[$adatafield[5]];
		
		autofueltankentry($tankid, $gallonsexchanged);
		?>
