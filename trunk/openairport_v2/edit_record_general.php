...
// There is a Record ID Supplied, Do work
				...
				if ($objrs) {
						...
						//	echo $tmpid;
							...
							<tr>
								...
								<td class="formanswers" id="<?=$adatafield[$i]?>ajax" name="<?=$adatafield[$i]?>ajax">
									<?
									switch ($ainputtype[$i]) {
									...
									<TEXTAREA class="Commonfieldbox" name="<?=$adatafield[$i];?>" rows="10" cols="60"><?=$objarray[$adatafield[$i]];?></TEXTAREA>
											...
											case "SELECT":	// if user entered "SELECT" as the input type make a select box
													switch ($aheadername[$i]) {	
															case "Type of Lease":
																	$adataselect[$i]("all", "all", $adatafield[$i], "show",  $objarray[$adatafield[$i]],$adatafield[$i+1]);
																	break;
															case "Item Leased":
																	//echo "Value ".$objarray[$adatafield[$i-1]]." Value";
																	$adataselect[$i]("all", "all", $adatafield[$i], "show",  $objarray[$adatafield[$i]], $objarray[$adatafield[$i-1]]);
																	break;
															default:
							...
							<tr>
								...
								<td class="formanswers">
									<INPUT class="formsubmit" TYPE="button" VALUE="Map Pavement Location" onClick="openChild2('mapdrawit.php?xpoints=<?=$adatafield[$i]?>&ypoints=<?=$adatafield[$i+1];?>','Win2')">
									</td>
								</tr>
											<?
										}
		...
		$dutylogevent	= "Edited record ID:".$_POST["recordid"]." in table ".$_POST['tbltextsorttable']."";
