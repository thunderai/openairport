...
<div id="container" style="position: relative; width: 1800px">
										<?
										$t6 = timer();
										$atimer[0] = timer();
										$counter = 1;
										while ($objarray = mysqli_fetch_array($objrs, MYSQLI_ASSOC)) {
												...
												?>
	<div id="container" style="position: relative; width: 1800px;top:<?=($counter*28);?>">
...
<table border="1" cellpadding="5" cellspacing="1">
	...
	<tr>
		<td>
			Timer Location 1 (initial Timer at start of page)
			<td>
		<td>
			<?=$t1;?>
			</td>
	...
	<tr>
		<td>
			Timer Location 2 (after the Include Files)
			<td>
		<td>
			<?=$t2;?>
			</td>
	...
	<tr>
		<td>
			Timer Location 3 (after Variables are set)
			<td>
		<td>
			<?=$t3;?>
			</td>
	...
	<tr>
		<td>
			Timer Location 4 (after array serailization)
			<td>
		<td>
			<?=$t4;?>
			</td>
	...
	<tr>
		<td>
			Timer Location 5 (after SQL Statement Construction)
			<td>
		<td>
			<?=$t5;?>
			</td>
	...
	<tr>
		<td>
			Timer Location 6 (Before Records Loop)
			<td>
		<td>
			<?=$t6;?>
			</td>
	...
	<tr>
		<td>
			Timer Location 7 (After the Record connection is closed)
			<td>
		<td>
			<?=$t7;?>
			</td>
...
<?
$tmpaverage	= 0;
$tmpcounter	= count($atimer);
for ($i=0; $i<$tmpcounter; $i=$i+1) {
	if ($i==0) {
			echo "0 | ";
		}
		else {
			$tmpvalue = ($atimer[$i] - $atimer[$i-1]);
			$tmpaverage = $tmpaverage + $tmpvalue;
			echo $tmpvalue." | ";
		}
	}
echo "<br>".(($tmpaverage) / $tmpcounter)."<br>";
?>
