<?php
// The purpose of this function is to provide a form control to list different animal species you will find durring a wildlife hazard management report.
function part139337_combobox_animalspecieswall($suppliedid, $archived, $nameofinput, $showcombobox, $default) {
	...
	if ($archived == "all") {																	// if supplied 'all' for the archived variable do the following
			...
			if ($archived=="yes") {																// If archived is 'yes' then
					if ($tmp_flagger==0) {
							$nsql = "WHERE tbl_139_337_sub_s.139337_sub_s_archived_yn = 1 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND tbl_139_337_sub_s.139337_sub_s_archived_yn = 1 ";
							$sql = $sql.$nsql;
						}
				}
				else {
					if ($tmp_flagger==0) {
							$nsql = "WHERE tbl_139_337_sub_s.139337_sub_s_archived_yn = 0 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND tbl_139_337_sub_s.139337_sub_s_archived_yn = 0 ";
							$sql = $sql.$nsql;
						}
				}
		}
