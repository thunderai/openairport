...
function gs_eventtypes_silent($suppliedid, $archived, $nameofinput, $showcombobox, $default) {
	...
	$sql = "SELECT * FROM tbl_general_eventtypes ";										// start the SQL Statement with the common syntax
	...
	if ($suppliedid=="all") {																	// if supplied 'all' for the menu_id so the following
			...
			$nsql = "WHERE `event_type_id` = ".$suppliedid." ";								// if supplied a menu_id, then add it to the SQL Statement
	...
	if ($archived == "all") {																	// if supplied 'all' for the archived variable do the following
			...
			if ($archived=="yes") {																// If archived is 'yes' then
					if ($tmp_flagger==0) {
							$nsql = "WHERE tbl_general_eventtypes.event_type_archived_yn = 1 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND tbl_general_eventtypes.event_type_archived_yn = 1 ";
							$sql = $sql.$nsql;
						}
				}
				else {
					if ($tmp_flagger==0) {
							$nsql = "WHERE tbl_general_eventtypes.event_type_archived_yn = 0 ";
							$sql = $sql.$nsql;
							$tmp_flagger = 1;
						}
						else {
							$nsql = "AND tbl_general_eventtypes.event_type_archived_yn = 0 ";
							$sql = $sql.$nsql;
						}
				}
		}
	$sql = $sql." ORDER BY event_type_name ";
	//echo $sql;
	...
	<SELECT class="Commonfieldbox" name="<?php echo $nameofinput;?>">
					...
					while ($objfields = mysqli_fetch_array($objrs_support, MYSQLI_ASSOC)) {
							...
							$tmpsuppliedid 		= $objfields['event_type_id'];
							$tmpsuppliedname 	= $objfields['event_type_name'];
							$tmpsuppliedarch	= $objfields['event_type_archived_yn'];
